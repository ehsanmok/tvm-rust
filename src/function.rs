...
// let func = function::Builder()::default()::get_function("name").args(&[...])
...
impl<'a> Builder<'a> {
    pub fn new(func: Option<Function>, arg_buf: Option<Box<[TVMArgValue<'a>]>>) -> Self {
        Self { func, arg_buf }
    }
        ...
        TypeCode: From<&'b T>,
    {
        ...
        if self.arg_buf.is_none() {
            ...
            Self::new(self.func, new_arg_buf)
        }
    }
